...
export function Navbar({ onOpenMobileMenu }: NavbarProps) {
  ...
  return (<header className="sticky top-0 z-40 w-full border-b-1.5 border-slate-900 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md"> <div className="flex items-center justify-between h-16 px-4 sm:px-6 gap-4">
    ...
    <div className="flex items-center gap-2 shrink-0">
      ...
      <button
        ...
        className={`hidden md:inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-lg border transition-colors ${error
          ...
          <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="py-1">
              ...
              <Link
                href="/settings"
                onClick={() => setIsProfileOpen(false)}
                className="flex items-center gap-2.5 px-4 py-2 text-xs font-medium text-slate-700 dark:hover:text-slate-100 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              >
